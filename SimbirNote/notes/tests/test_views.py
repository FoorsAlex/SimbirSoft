...
from ..models import Comment, Follow, Group, Note
...
class TestPages(TestCase):
    ...
    def test_pages_uses_correct_template(self):
        ...
        templates_pages_names = {
            'notes/save_notes.html': reverse('notes:index'),
            'notes/group_list.html': reverse(
                'notes:group_list', kwargs={'slug': self.group.slug}
            ),
            'notes/profile.html': reverse(
                'notes:profile', kwargs={'username': self.user_auth.username}
            ),
            'notes/note_detail.html': reverse(
                'notes:post_detail', kwargs={'post_id': self.post.pk}
            ),
            'notes/create_note.html': reverse(
                'notes:post_edit', kwargs={'post_id': self.post.pk}
            ),
            'notes/follow.html': reverse('notes:follow_index')
        }
        for template, reverse_name in templates_pages_names.items():
            with self.subTest(reverse_name=reverse_name):
                response = self.client_authorized.get(reverse_name)
                self.assertTemplateUsed(response, template)
        response = self.client_authorized.get(reverse('notes:post_create'))
    ...
    def test_pages_with_paginator_show_correct_context(self):
        ...
        pages_kwargs = {
            'notes:group_list': ('slug', self.group.slug),
            'notes:profile': ('username', self.user_auth.username),
            'notes:index': (),
            'notes:follow_index': ()
        }
        for reverse_name, kwargs in pages_kwargs.items():
            with self.subTest(reverse_name=reverse_name):
                if reverse_name == 'notes:index' or reverse_name == 'notes:follow_index':
    ...
    def test_post_detail_pages_show_correct_context(self):
        ...
        response = (self.client_authorized.get(
            reverse('notes:post_detail', kwargs={'post_id': self.post.pk})
        )
        )
    ...
    def test_first_page_contains_ten_records(self):
        ...
        for reverse_url, kwargs in reverse_list.items():
            with self.subTest(reverse_url=reverse_url):
                if reverse_url == 'notes:profile' or reverse_url == 'notes:group_list':
                    response = self.client_authorized.get(
...
class TestCash(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.reverse_index = reverse('notes:index')
...
class TestFollow(TestCase):
    ...
    def test_follow_index_user(self):
        response = self.authorized_client.get(reverse('notes:follow_index'))
