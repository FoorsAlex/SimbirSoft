...
class TestComment:
    ...
    def test_comment_add_view(self, client, post):
        try:
            response = client.get(f'/notes/{post.id}/comment')
        except Exception as e:
            assert False, f'''Страница `/notes/<post_id>/comment/` работает неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302) and response.url == f'/notes/{post.id}/comment/':
            url = f'/notes/{post.id}/comment/'
        else:
            url = f'/notes/{post.id}/comment'
        assert response.status_code != 404, (
            'Страница `/notes/<post_id>/comment/` не найдена, проверьте этот адрес в *urls.py*'
        )
        ...
        if not(response.status_code in (301, 302) and response.url.startswith('/auth/login')):
            assert False, (
                ...
                '`/notes/<post_id>/comment/` отправляете на страницу авторизации'
            )

    @pytest.mark.django_db(transaction=True)
    def test_comment_add_auth_view(self, user_client, post):
        try:
            response = user_client.get(f'/notes/{post.id}/comment')
        except Exception as e:
            assert False, f'''Страница `/notes/<post_id>/comment/` работает неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302) and response.url == f'/notes/{post.id}/comment/':
            url = f'/notes/{post.id}/comment/'
        ...
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/notes/<post_id>/comment/` '
        ...
        assert comment is not None, (
            'Проверьте, что вы создаёте новый комментарий `/notes/<post_id>/comment/`'
        )
        assert response.url.startswith(f'/notes/{post.id}'), (
            'Проверьте, что перенаправляете на страницу поста '
            '`/notes/<post_id>/` после добавления нового комментария'
        )
