...
class TestPostView:
    ...
    def test_post_view_get(self, client, post_with_group):
        try:
            response = client.get(f'/notes/{post_with_group.id}')
        except Exception as e:
            assert False, f'''Страница `/notes/<post_id>/` работает неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302):
            response = client.get(f'/notes/{post_with_group.id}/')
        assert response.status_code != 404, (
            'Страница `/notes/<post_id>/` не найдена, проверьте этот адрес в *urls.py*'
        )

        post_context = get_field_from_context(response.context, Post)
        assert post_context is not None, (
            'Проверьте, что передали статью в контекст страницы `/notes/<post_id>/` типа `Post`'
        )

        try:
            from posts.forms import CommentForm
        except ImportError:
            assert False, 'Не найдена форма CommentForm в notes.form'

        comment_form_context = get_field_from_context(response.context, CommentForm)
        assert comment_form_context is not None, (
            'Проверьте, что передали форму комментария в контекст страницы `/notes/<post_id>/` типа `CommentForm`'
        )
        assert len(comment_form_context.fields) == 1, (
            'Проверьте, что форма комментария в контекстке страницы `/notes/<post_id>/` состоит из одного поля'
        )
        assert 'text' in comment_form_context.fields, (
            'Проверьте, что форма комментария в контекстке страницы `/notes/<post_id>/` содержится поле `text`'
        )
        assert type(comment_form_context.fields['text']) == forms.fields.CharField, (
            'Проверьте, что форма комментария в контекстке страницы `/notes/<post_id>/` '
            'содержится поле `text` типа `CharField`'
        )
        assert hasattr(post_context, 'image'), (
            'Убедитесь, что статья, передаваемая в контекст страницы `/notes/<post_id>/`, имеет поле `image`'
        )
...
class TestPostEditView:
    ...
    def test_post_edit_view_get(self, client, post_with_group):
        try:
            response = client.get(f'/notes/{post_with_group.id}/edit')
        except Exception as e:
            assert False, f'''Страница `/notes/<post_id>/edit/` работает неправильно. Ошибка: `{e}`'''
        if (
                response.status_code in (301, 302)
                and not response.url.startswith(f'/notes/{post_with_group.id}')
        ):
            response = client.get(f'/notes/{post_with_group.id}/edit/')
        assert response.status_code != 404, (
            'Страница `/notes/<post_id>/edit/` не найдена, проверьте этот адрес в *urls.py*'
        )

        assert response.status_code in (301, 302), (
            'Проверьте, что вы переадресуете пользователя со страницы '
            '`/notes/<post_id>/edit/` на страницу поста, если он не автор'
        )

    @pytest.mark.django_db(transaction=True)
    def test_post_edit_view_author_get(self, user_client, post_with_group):
        try:
            response = user_client.get(f'/notes/{post_with_group.id}/edit')
        except Exception as e:
            assert False, f'''Страница `/notes/<post_id>/edit/` работает неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302):
            response = user_client.get(f'/notes/{post_with_group.id}/edit/')
        ...
        assert any([post_context, postform_context]) is not None, (
            'Проверьте, что передали статью в контекст страницы `/notes/<post_id>/edit/` типа `Post` или `PostForm`'
        )

        assert 'form' in response.context, (
            'Проверьте, что передали форму `form` в контекст страницы `/notes/<post_id>/edit/`'
        )
        fields_cnt = 3
        assert len(response.context['form'].fields) == fields_cnt, (
            f'Проверьте, что в форме `form` на страницу `/notes/<post_id>/edit/` {fields_cnt} поля'
        )
        assert 'group' in response.context['form'].fields, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` есть поле `group`'
        )
        assert type(response.context['form'].fields['group']) == forms.models.ModelChoiceField, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` поле `group` типа `ModelChoiceField`'
        )
        assert not response.context['form'].fields['group'].required, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` поле `group` не обязательно'
        )

        assert 'text' in response.context['form'].fields, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` есть поле `text`'
        )
        assert type(response.context['form'].fields['text']) == forms.fields.CharField, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` поле `text` типа `CharField`'
        )
        assert response.context['form'].fields['text'].required, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` поле `group` обязательно'
        )

        assert 'image' in response.context['form'].fields, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` есть поле `image`'
        )
        assert type(response.context['form'].fields['image']) == forms.fields.ImageField, (
            'Проверьте, что в форме `form` на странице `/notes/<post_id>/edit/` поле `image` типа `ImageField`'
        )
    ...
    def test_post_edit_view_author_post(self, mock_media, user_client, post_with_group):
        ...
        try:
            ...
            assert False, f'''Страница `/notes/<post_id>/edit/` работает неправильно. Ошибка: `{e}`'''
        url = (
            f'/notes/{post_with_group.id}/edit/'
            if response.status_code in (301, 302)
            else f'/notes/{post_with_group.id}/edit'
        )
        ...
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/notes/<post_id>/edit/` '
        ...
        assert post is not None, (
            'Проверьте, что вы изменили пост при отправки формы на странице `/notes/<post_id>/edit/`'
        )
        assert response.url.startswith(f'/notes/{post_with_group.id}'), (
            'Проверьте, что перенаправляете на страницу поста `/notes/<post_id>/`'
        )
